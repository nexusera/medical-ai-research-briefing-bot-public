...
2. **1:1 剖析对齐**: 论文列表中的**每一篇**论文都必须在 `## 🔬 深度剖析` 章节中拥有专属的剖析小节。严禁遗漏。
3. **精准溯源**: 每一条研究必须带有**原文标题**作为二级/三级标题，并紧随其后提供**原文链接**。
4. **专家剖析维度 (Expert Analysis Dimensions)**: 每一篇论文的剖析必须涵盖以下 5 个核心维度：
   - **主题关联 (Context)**: 论文如何契合当前的检索方向。
   - **痛点解决 (Problem Solved)**: 该研究具体解决了哪些此前未被攻克的难题。
   - **优缺点分析 (Pros/Cons)**: 评估技术/临床增益与局限（如计算开销、偏差）。
   - **现有对比 (SOTA Comparison)**: 与领域内现有论文（如有）的横向对比。
   - **改进逻辑 (Actionable Insights)**: 给出前瞻性的改进建议或与其他技术结合的思路提示。
