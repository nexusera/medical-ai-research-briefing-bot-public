...
### 扩展模式：Related 检索策略（非“最新”，只求“相关”）

当用户的核心意图是索要**相关工作、对比基线、经典代表作**而非“今日更新”时，必须解除上述所有时效性时间窗（Fallback A/B 失效）：
- **抛弃硬窗口**：取消 `24h/72h` 限制。
- **软约束优先**：遵循“任务匹配度 > 对照性 > 来源权重 > 近3年优先 > 影响力”进行漏斗筛选。
- **允许回溯基线**：如提取的为该方向的经典奠基模型（>3年），独立打上 `[Seminal / Baseline]` 标签。
- **控量输出**：每个追踪方向只需展示 3-7 篇最具代表性的拼图（涵盖 1-2 篇基线，1-3 篇近年 SOTA，1-2 篇对立路线）。
