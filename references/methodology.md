...
## 3. 全局降噪与筛选逻辑 (Signal-to-Noise Rules)

在采集环节，必须执行以下过滤准则以确保播报的是“一线研究”而非“工程噪声”：

### ✅ 优先收录 (Signals)
1. **多机构数据**: 跨中心验证的研究具有更高的置信度。
2. **明确实验设计**: 必须有对照组 (Control) 或逻辑严密的消融实验。
3. **量化临床指标**: 必须提供具体数值（如 F1 提升 %、CER 降低值）。

### ❌ 降级或忽略 (Noise)
1. **单中心小样本**: 无外部验证且样本量不足。
2. **纯概念讨论**: 缺乏代码或实证支撑的 Vision 类文章。
3. **Marketing 宣传**: 缺乏方法论透明度的商业推销式论文。
